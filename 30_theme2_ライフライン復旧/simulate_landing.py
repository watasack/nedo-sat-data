...
OPS = [
    ...
    ("③", "キャプションを短縮する",
     ...
     "図3 消雪日を特定できる年数と、蔵王山の実測。"),
    # ③は J-15（最悪年48日の開示）で本文が7字伸びたので、(B)ではもう1手要る。
    # 落とすのは「（誤差±10日）」——中央値20日から読者が導けるうえ、キャプションにも「±10日」が残る。
    # **「最悪の年は48日」は落とさない**（自白の側を削ると J-15 の趣旨が消える）。
    ("③", "「（誤差±10日）」を落とす（中央値20日から導けるうえ、図3のキャプションにも残る）",
     "観測ギャップの中央値は20日（誤差±10日）、最悪の年は48日である。",
     "観測ギャップの中央値は20日、最悪の年は48日である。"),
    ("①", "気象庁336地点の文を削る（同じ事実は④にある）",
     "気象庁が積雪深を測る336地点の最高は1,292mで、火口より937〜2,916m低い。", ""),
    # ⑤の「火山災害警戒地域…」と那須岳の業務正式名称は、2026年8月10日の V-18 修正で
    # **(A)の本文から既に落とした**（マグマ噴火期の一文を入れる原資にした）。
    # したがって(B)ではもう抜けない。代わりに次の2つを使う。
    ("⑤", "想定ユーザーの一文から府省名を落とす（次段落に「国の火山土砂災害対策」があるので所管は残る）",
     "想定ユーザーは国土交通省の砂防事務所と都道府県の砂防部局である。",
     "想定ユーザーは砂防事務所と都道府県の砂防部局である。"),
    ("⑤", "那須岳段落の締めを圧縮する（前後の関係は直前の2文で既に読める）",
     "本提案が担うのは当年値では作れない多年の年超過確率分布で、両者は前後の関係にある。",
     "本提案が担うのは当年値では作れない多年の年超過確率分布である。"),
    ("⑤", "2027年度の一文から対象名を落とす（直前の文が予算行の原文で「計画の改定」「リアルタイム"
     "ハザードマップの運用・整備の推進」を引いているので、何の改定かは残る）",
     "2027年度は計画とハザードマップ改定の検討業務へ技術提供し、",
     "2027年度は改定の検討業務へ技術提供し、"),
    ("⑤", "キャプションを短縮する",
