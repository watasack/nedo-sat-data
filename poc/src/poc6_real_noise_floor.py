...
検出力は 1/sqrt(n) で効くので、これだけで最小検知段差が sqrt(14/6)=1.53 倍改善する。
...
results["epoch_recovery"] = dict(
    ...
    検出力の改善倍率=round(float(np.sqrt(n_ep / max(sum(1 for v in ref_fr if v >= 0.5), 1))), 2),
)
print("=== 1. 参照面の置き場所 ===")
...
print(f"  → 参照面を共通有効域に置けばエポックは "
      ...
      f"（検出力 ×{results['epoch_recovery']['検出力の改善倍率']}）")
